...
class Covid19Adapter(LogicAdapter):
    ...
    def process(self, input_statement,additional_response_selection_parameters):
        ...
        if any(x in input_statement.text.split() for x in words):
            if any(y in input_statement.text.split() for y in Countries):
                ...
                if len(answer) == 0:
                    response_statement = Statement(text='Ask again but please include the Country name (in some cases, Name of country have"-" insight, ex: viet-nam)')    
                    answer.append(str(response_statement))
