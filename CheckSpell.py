...
class SpellChecker(LogicAdapter):
  ...
  def __init__(self,chatbot, **kwargs):
    ...
    words = []
    for filename in glob.glob('E:/fpt/5/CSD301/Final Project/Spell-Checker-master/library.txt'):
  ...
  def can_process(self, statement):
      words = ['check','check:','check-spell:', 'correct-spell:', 'Turn-on-Spell-Checker:']
  ...
  def process(self,word_input,additional_response_selection_parameters):
    ...
    for i in word:
      if i in vocab:
        ...
        show=[x[0] for x in sorted([(c, self.word_probas[c]) for c in valid_candidates], key=lambda tup: tup[1], reverse=True)[0:5]]
       # response_statement = Statement(text='{}'.format(sorted([(c) for c in valid_candidates], key=lambda tup: tup[1], reverse=True)[0:5])) 
        response_statement = Statement(text='{}'.format(show))       
